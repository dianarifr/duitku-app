  ...
  return (
    <div className="min-h-screen pb-10 font-sans bg-white">
      ...
      <form onSubmit={handleSave} className="p-8 space-y-8">
        ...
        <div className="space-y-3">
          ...
          <div className={`grid ${categories.length > 0 ? 'grid-cols-4' : 'grid-cols-1'} gap-3 p-1 overflow-y-auto max-h-64 custom-scrollbar`} style={isShaking ? { animation: 'shake 0.1s ease-in-out 0s 10' } : {}}>
            {categories.length > 0 ? (
              ...
              <button type="button" onClick={() => setCurrentPage('kategori')} className="p-8 text-center transition-all border-2 border-blue-200 border-dashed bg-blue-50 rounded-xl group active:scale-95">
        ...
        <div className="grid grid-cols-1 gap-6 text-left sm:grid-cols-2">
          <div>
            ...
            <div className="relative mt-1">
              <input type="date" value={formData.date} onClick={(e) => e.target.showPicker()} onChange={(e) => setFormData({...formData, date: e.target.value})} className="w-full p-4 font-bold text-gray-700 border-none outline-none bg-gray-50 rounded-xl focus:ring-2 focus:ring-blue-500" />
        ...
        {/* SUBMIT BUTTON */}
        <button disabled={loading} className={`w-full py-5 rounded-xl text-white font-black text-lg shadow-xl active:scale-95 transition-all mt-6 ${activeTab === 'pemasukan' ? 'bg-green-600 shadow-green-100' : 'bg-red-600 shadow-red-100'} ${loading ? 'opacity-50' : ''}`}>
